    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        if x > width/2:
            if y > height/2:
                ...
                point_botton_right = (width-1, height-1)
            else:
                point_top_left = (int(width/2),0)
                point_botton_right = (width-1, int(height/2))
        else:
            if y > height/2:
                point_top_left = (0,int(height/2))
                point_botton_right = (int(width/2), height-1) 
            else:
                point_top_left = (0,0)
                point_botton_right = (int(width/2),int(height/2))
        cv2.rectangle(img,(0,0), (width-1, height-1), (255,255,255),-1)
        cv2.rectangle(img,point_top_left, point_botton_right,(135,120,180),-1)
